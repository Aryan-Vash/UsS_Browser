  ...
  for (var i = 0; i < grantedPermissions.length; i++) {
    if (requestOrigin === grantedPermissions[i].origin) {
      ...
      if (requestPermission === 'media' && grantedPermissions[i].permission === 'media') {
        ...
        // MODIFICATION: Check both details.mediaType and details.mediaTypes[0]
        const requestMediaType = requestDetails.mediaType || (requestDetails.mediaTypes && requestDetails.mediaTypes[0])
        if (requestMediaType && grantedPermissions[i].details.mediaTypes.includes(requestMediaType)) {
          return true
        }
    ...
    */
    if (isPermissionGrantedForOrigin(requestOrigin, permission, details)) {
      ...
      */
      ...
      // --- START: MODIFIED CODE - SPLIT MEDIA REQUESTS ---

      // This is a multi-item media request. Split it.
      if (permission === 'media' && details.mediaTypes && details.mediaTypes.length > 1) {
        
        let responses = {}
        let requiredTypes = details.mediaTypes

        // This master callback will collect decisions for all split requests
        // and issue the final single callback to Electron
        function masterCallback (mediaType, decision) {
          responses[mediaType] = decision
          // If we have responses for all types
          if (Object.keys(responses).length === requiredTypes.length) {
            // Grant if *all* sub-requests were granted, else deny.
            const finalDecision = requiredTypes.every(type => responses[type] === true)
            callback(finalDecision)
          }
        }

        // Create a separate pending permission for each media type
        details.mediaTypes.forEach(mediaType => {
          const newDetails = { ...details, mediaTypes: [mediaType], mediaType: mediaType }
          
          // Create a *new* callback for this sub-permission
          const subCallback = (decision) => masterCallback(mediaType, decision)
          ...
          pendingPermissions.push({
            ...
            contents: webContents,
            origin: requestOrigin,
            permission: 'media', // Still 'media'
            details: newDetails, // But with *single* mediaType
            callback: subCallback // Use the special sub-callback
          })
          nextPermissionId++
        })

        sendPermissionsToRenderers() // Send all new split permissions

      } else {
        // Old logic: just push the single request
        pendingPermissions.push({
          permissionId: nextPermissionId,
          tabId: getTabIDFromWebContents(webContents),
          contents: webContents,
...
// In permissionManager.js
ipc.on('handle-permission-request', function (e, args) {
  console.log('\n[DEBUG] "handle-permission-request" RECEIVED. Args:', args)
  let found = false

  for (var i = 0; i < pendingPermissions.length; i++) {
    if (args.permissionId && pendingPermissions[i].permissionId === args.permissionId) {
      
      found = true
      const request = pendingPermissions[i]
      
      console.log(`[DEBUG] Found matching permission ID: ${request.permissionId}.`)
      console.log(`[DEBUG] The callback is a: ${typeof request.callback}`)
      if (typeof request.callback === 'function') {
        request.callback(args.grant)
        console.log(`[DEBUG] Callback was called with: ${args.grant}`)
      } else {
        console.error(`[DEBUG] ERROR: Callback for ${request.permissionId} is NOT a function!`)
      }
      
      if (args.grant) {
        console.log('[DEBUG] Permission was granted. Moving to grantedPermissions.')
        request.granted = true
        grantedPermissions.push(request)
      }
