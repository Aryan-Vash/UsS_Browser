...
function normalizeAnalysisResponse (data) {
  ...
  if (Array.isArray(data)) {
    return data[0] || { label: -1 }
  }

  if (Array.isArray(data.prediction)) {
    return data.prediction[0] || { label: -1 }
  }
...
function coerceLabel (labelValue) {
  const numericLabel = typeof labelValue === 'number' ? labelValue : parseInt(labelValue, 10)
  return Number.isFinite(numericLabel) ? numericLabel : -1
}
...
ipc.on('analyze-url', (event, { url, channel }) => {
  ...
  fetch(apiUrl, {
    ...
    .then(res => {
      console.log(`[MainProcess] API response status: ${res.status}`)
      if (!res.ok) {
    ...
    .then(data => {
      console.log(`[MainProcess] Raw API response data:`, data)
      const normalized = normalizeAnalysisResponse(data)
      const label = coerceLabel(normalized.label)
      const payload = { ...normalized, label }
      console.log('[MainProcess] Normalized payload:', payload)
      event.sender.send(channel, payload)
    })
    .catch(err => {
      console.error('[MainProcess] API request failed:', err)
      event.sender.send(channel, { label: -1 }) // Indicate error
    })
})
...
ipc.handle('security-report-fetch', async (event, url) => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Security report API returned status ${response.status}`)
    }

    const data = await response.json()
    console.log('[MainProcess] Security report response:', data)
    return data
  } catch (error) {
    console.error('[MainProcess] Failed to fetch security report:', error)
    throw error
  }
})
